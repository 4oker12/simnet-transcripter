#!/bin/bash
set -euo pipefail

if [[ $# -lt 1 || $# -gt 3 ]]; then
  echo "Usage: $0 FILE [PROFILE] [LANGUAGE]" >&2
  exit 2
fi

file="$1"
profile="${2:-}"
language="${3:-auto}"
base_url="${SIMNET_TRANSCRIBER_URL:-http://127.0.0.1:8000}"

if [[ ! -f "$file" ]]; then
  echo "File not found: $file" >&2
  exit 2
fi

args=(-fsS --max-time 3600 -X POST -F "file=@$file" -F "language=$language")
[[ -z "$profile" ]] || args+=(-F "profile=$profile")

response_file="$(mktemp /tmp/simnet-transcribe.XXXXXX)"
trap 'rm -f "$response_file"' EXIT
http_code="$(curl "${args[@]}" -o "$response_file" -w '%{http_code}' "$base_url/transcribe" || true)"

if [[ "$http_code" != "200" ]]; then
  echo "Transcription failed (HTTP ${http_code:-connection error}):" >&2
  jq -r '.detail // .' "$response_file" 2>/dev/null >&2 || sed -n '1,20p' "$response_file" >&2
  exit 1
fi

if [[ "${SIMNET_TRANSCRIBE_JSON:-0}" == "1" ]]; then
  jq . "$response_file"
  exit 0
fi

jq -r '"Request: \(.request_id)\nProfile: \(.profile)\nLanguage: \(.language) (p=\(.language_probability // "n/a"))\nDuration: \(.duration_seconds) s\nProcessing: \(.processing_seconds) s\nRealtime factor: \(.realtime_factor // "n/a")\nAudio SHA-256: \(.audio_sha256 // "n/a")\n\nTranscript:\n\(.text)"' "$response_file"
